...
export default function Topbar({ title, subtitle, notifCount = 0, showSearch = false }) {
  ...
  return (
    <header className="flex items-center gap-4 border-b border-white/10 bg-[#8C0404] px-4 py-3.5 backdrop-blur lg:px-6">
      <button className="rounded-lg p-2 text-white/80 hover:bg-white/10 lg:hidden">
        <Menu size={20} />
      </button>
      ...
      {showSearch && (
        <div className="hidden max-w-sm flex-1 items-center gap-2 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white/60 md:flex">
          <Search size={16} />
      ...
      <div className="relative">
        ...
        >
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-xs font-semibold text-[#8C0404]">
            {meta.initials}
          </div>
