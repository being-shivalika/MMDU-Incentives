...
const ResearchReviewDashboard = () => {
  ...
  const loadSubmissions = async () => {
    ...
    try {
      ...
      const queue = data.filter((submission) => {
        if (user?.role === "rd_cell") {
          return submission.status === "Pending R&D";
        }

        if (user?.role === "rpc_cell") {
          return submission.status === "Pending RPC";
        }
  ...
  // Compute stats
  const stats = {
    pendingAnalysis: submissions.filter(
      (s) => s.status && s.status.includes("Pending"),
    ).length,
    underReview: submissions.filter(
      (s) => s.status === "Under Review" || s.status === "RPC Review",
    ).length,
    readyForAccounts: submissions.filter((s) => s.status === "Approved").length,
    returnedForClarification: submissions.filter(
      (s) => s.status === "Revision Requested" || s.status === "Returned",
    ).length,
